...
#     derivatives = xp.zeros((tt_batch.batch_size, tt_batch.order))
#     right_parts, left_parts = [], []
...
#         )
#         left_parts.append(left_part)
#         derivatives[:, i] = partial_derivative.contract(batch=True).squeeze().view(xp.ndarray)

#     return derivatives

def multi_derivative(tt: TensorTrain, phis, dphis):
    batch_size = len(phis[0])
    phis_batch = TensorNetwork(cores=phis, names=[f"phi_{i}" for i in range(len(phis))])
    dphis_batch = TensorNetwork(
        cores=dphis, names=[f"dphi_{i}" for i in range(len(dphis))]
    )

    derivative = xp.zeros((batch_size, tt.order))
    right_parts, left_parts = [], []
    ...
    for i in range(tt.order):
        ...
        left_parts.append(left_part)
        derivative[:, i] = partial_derivative.contract(batch=True).squeeze()
    return derivative