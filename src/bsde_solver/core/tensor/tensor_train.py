...
#             core = TensorCore(
#                 xp.zeros((batch_size, ranks[i], shape[i], ranks[i+1])),
...
#                 L = core_prev.unfold(0, (1, 2), 3).view(xp.ndarray)
#                 R = core_curr.unfold(0, (2, 3), 1).view(xp.ndarray)

#                 V, U = batch_qr(R)
#                 W = xp.einsum("bij,bjk->bik", U, R)
...
class TensorTrain(TensorNetwork):
    ...
    def __init__(self, shape: list[int], ranks: list[int]):
        ...
        for i in range(len(shape)):
            core = TensorCore(
                xp.zeros((ranks[i], shape[i], ranks[i+1])),
    ...
    def from_tensor(tensor, ranks: list[int]):
        ...
        Args:
            tensor (xp.ndarray): Tensor to create the tensor train from.
            orthogonality (str, optional): Orthogonality of the tensor train. Defaults to "right".
