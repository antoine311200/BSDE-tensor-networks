...
if __name__ == "__main__":
    ...
    for batch_size in batch_sizes[::-1]:
        ...
        print(f"Number of samples: {batch_size}, Explicit time: {total_time_explicit:.2f}s, Implicit time: {total_time_implicit:.2f}s")
    ...
    plt.plot(batch_sizes, errors_explicit[::-1], label="Explicit")
    plt.plot(batch_sizes, errors_implicit[::-1], label="Implicit")
    plt.xlabel("Batch size")
    plt.ylabel("Error")
    plt.title("Error comparison between explicit and implicit schemes for different batch sizes")
    plt.legend()
    plt.savefig("batch_size_error_comparison.png")
    plt.show()
