...
if __name__ == "__main__":
    ...
    print(f"Explicit time: {total_time_explicit:.2f}s, Implicit time: {total_time_implicit:.2f}s")

    plt.figure()
    plt.plot(mean_relative_errors_explicit[::-1], label="Explicit")
    plt.plot(mean_relative_errors_implicit[::-1], label="Implicit")
    plt.xlabel("Step")
    plt.ylabel("Mean relative error")
    plt.title("Mean relative error")
    plt.legend()
    plt.savefig("mean_relative_error.png", dpi=300)
    plt.show()